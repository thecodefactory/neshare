...
nemsgFileRequestAck::nemsgFileRequestAck(ncSocket *sock)
{
    m_sock = sock;
}

nemsgFileRequestAck::~nemsgFileRequestAck()
{
    m_sock = (ncSocket *)0;
}

int nemsgFileRequestAck::send(unsigned long fileSendID, 
...
{
    int ret = 1;
    int filenameLen = (filename ? MIN(strlen(filename),NE_MSG_MAX_DATA_LEN) : 0);

    if (filenameLen)
    {
        NEMSGFILEREQUESTACK_HEADER msgHeader;
        memset(&msgHeader,0,sizeof(NEMSGFILEREQUESTACK_HEADER));

        msgHeader.msgType = htonl(NE_MSG_FILE_REQUEST_ACK);
        ...
        if (m_sock->writeData((void *)&msgHeader,
                              sizeof(NEMSGFILEREQUESTACK_HEADER)) == NC_OK)
        {
...
int nemsgFileRequestAck::recv(unsigned long *fileSendID, 
                              unsigned long *maxBlockSize,
                              unsigned long *filesize,
                              unsigned char md5checksum[16],
                              char *filename,
                              unsigned long maxlen)
{
                         ...
                         sizeof(NEMSGFILEREQUESTACK_HEADER)) == NC_OK)
    {
        ...
        if (msgHeader.msgType == NE_MSG_FILE_REQUEST_ACK)
        {
